          ...
          {["Product", "Features", "Pricing", "Analytics", "Resources"].map((link) => (
            <Link
              ...
              className={`text-xs font-semibold uppercase tracking-wider transition ${
                link === "Features" ? "text-indigo-400" : "text-slate-400 hover:text-slate-100"
              }`}
            >
              {link}
            </Link>
            ...
            {["Product", "Features", "Pricing", "Analytics", "Resources"].map((link) => (
              <Link
                ...
                className={`text-sm font-bold uppercase tracking-wider ${
                  link === "Features" ? "text-indigo-400" : "text-slate-300 hover:text-white"
                }`}
              >
                {link}
              </Link>
      ...
      <main className="w-full flex-1 max-w-7xl mx-auto px-6 py-12 md:py-20 relative z-10 space-y-16">
        <section className="text-center max-w-2xl mx-auto space-y-4">
          ...
          <p className="text-xs md:text-sm text-slate-400 font-medium leading-relaxed">
            Discover a comprehensive suite of tools built for speed, performance, and simplicity. Monitor redirects and customize domains effortlessly.
          </p>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            {
              icon: Shield,
              title: "Custom Domain Attachment",
              desc: "Deploy short links on your own brand.co domain. Custom brand links can elevate click-through response by up to 34%."
            },
            {
              icon: BarChart3,
              title: "Real-Time Tracking",
              desc: "Sync visitor clicks in real time. Inspect hourly click counts, geo aggregates, and browser types instantly."
            },
            {
              icon: QrCode,
              title: "Print Vector QR Codes",
              desc: "Every dynamic short URL creates a premium vector QR code automatically, ready to download for pamphlets or packages."
            },
            {
              icon: Calendar,
              title: "Link Expiration Schedules",
              desc: "Set automatic expiration timestamps to auto-disable limited-time offer codes or holiday campaigns dynamically."
            },
            {
              icon: Users,
              title: "Collaborator Workspaces",
              desc: "Invite marketing teams or developers to collaborate, manage domain listings, and review campaign charts together."
            },
            {
              icon: Terminal,
              title: "Developer API Access",
              desc: "Integrate URL shortening capabilities directly into your application databases using our clean, fast REST API."
            },
            {
              icon: Grid,
              title: "Bulk URL Processing",
              desc: "Generate hundreds of custom short URLs in a single batch, perfect for bulk SMS lists or marketing outreach."
            },
            {
              icon: Globe,
              title: "Detailed Geo Metrics",
              desc: "Identify your click concentrations by state or country, letting you adjust local campaigns dynamically."
            },
            {
              icon: Smartphone,
              title: "Mobile OS Profiling",
              desc: "See if visitors are on Desktop, iOS, or Android, helping you optimize application deep links easily."
            }
          ].map((feat, i) => (
            <motion.div
            ...
            >
              ...
              <div className="space-y-1">
                <h3 className="text-xs font-bold text-slate-200 uppercase tracking-widest">{feat.title}</h3>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2.5 text-xs text-slate-600 font-bold uppercase tracking-wider">
              ...
              <Link href="/features" className="text-indigo-400">Features</Link>
